...
flowchart TB
    ...
    subgraph PROGRAM ["Program/Process"]
        direction LR
        STDIN[stdin 0] --> PROC[🖥️] 
        PROC --> STDOUT[stdout 1]
        PROC --> STDERR[stderr 2]
    end
    
    subgraph OUTPUT ["Data Destinations"]
        TERM[Terminal]
        OUTFILE[Files]
        NEXTPROG[Other Programs]
    end
    
    INPUT -->|"Data flows IN"| STDIN
    STDOUT -->|"Normal output flows OUT"| OUTPUT
    STDERR -->|"Error messages flow OUT"| OUTPUT
```
